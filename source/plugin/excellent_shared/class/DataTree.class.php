...
<?php
	...
	*/
	...
	class DataTree{
		...
		//获取json字符串
		public function getOneTreeNodeJosn($type_id=NULL){
		...
		* 更新所有的菜单到数据库
			...
			*/
			...
			if((empty($this->data_array[$root->type_id]) && $root->type_id >= $this->ID) && !empty($father_type)){
				...
				$sql = "SELECT c.type_id as 'type_id' "
    				. " FROM `gcdiscuzforum_forum_gc_type_relation` b "
    				. "	LEFT JOIN `gcdiscuzforum_forum_gc_type_thread` c ON b.child_type_id=c.type_id "
				...
				if(empty($row)){
					...
					$sql = "INSERT INTO `gcdiscuzforum_forum_gc_type_thread`(`type_name`,`type_level`) VALUES ('".$root->type_name."',".$root->type_level.")";
					$sqldata=$this->mysqli->query($sql);
					if (!$sqldata) {
						var_dump($sqldata);
    					return false;
					...
					$sql = "INSERT INTO `gcdiscuzforum_forum_gc_type_relation`(`father_type_id`,`child_type_id`)VALUES(".$father_type->type_id.",".$type_id.")";
					$sqldata=$this->mysqli->query($sql);
					if (!$sqldata) {
						var_dump($sqldata);
    					return false;
				...
				if(!empty($this->data_array[$root->type_id]) && $this->data_array[$root->type_id]->type_name != $root->type_name){
					...
					$sql = "UPDATE `gcdiscuzforum_forum_gc_type_thread` SET `type_name`='".$root->type_name."' WHERE `type_id`=".$root->type_id.";";
					$sqldata=$this->mysqli->query($sql);
					if (!$sqldata) {
						var_dump($sqldata);
    					return false;
		...
		private function deleteOneNode(&$node){
			...
			//再删除本节点
			$sql = "DELETE FROM `gcdiscuzforum_forum_gc_type_thread` WHERE `type_id`=".$node->type_id.";";
			$sqldata=$this->mysqli->query($sql);
			if (!$sqldata) {
				var_dump($sqldata);
    			return false;
